...
"""
 Description: 
           Parallelized implementation of Q2n index. 
...
from scripts.IQA.onions_quality import onions_quality

def parallel_onions_quality(args):
    dat1, dat2, size1 = args
    return onions_quality(dat1, dat2, size1)

def parallel_q2n(I_GT, I_F, Q_blocks_size, Q_shift):
        ...
        with ProcessPoolExecutor() as executor:
            block_args = [
                (I_GT[j * Q_shift : (j * Q_shift) + Q_blocks_size, i * Q_shift : (i * Q_shift) + size2, :], 
                 ...
                 Q_blocks_size)
                for j in range(stepx) for i in range(stepy)
            ]

            results = list(executor.map(parallel_onions_quality, block_args))

            for idx, result in enumerate(results):
                j, i = divmod(idx, stepy)
                valori[j, i, :] = result
